...
#define CMD_GET_PATH 13 // Mới: Lấy đường dẫn của một node
#define CMD_ERROR 99
...
typedef struct
{
    ...
    int node_id;       // ID của node bị tác động (rename, delete, download, copy, move, share, get_path)
} RequestPacket;
...
// Cấu trúc Node trên đường dẫn (MỚI)
typedef struct
{
    int id;
    char name[256];
