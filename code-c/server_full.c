...
void get_username_by_id(MYSQL *conn, int user_id, char *out_name) {
    ...
    if (mysql_query(conn, query) == 0) {
        ...
        } else { strcpy(out_name, "Unknown"); }
        if (res) mysql_free_result(res);
    } else { strcpy(out_name, "Error"); }
}

void get_node_name_by_id(MYSQL *conn, int node_id, char *out_name) {
    if (node_id == 0) { strcpy(out_name, "Root"); return; }
    char query[256];
    ...
    if (mysql_query(conn, query) == 0) {
        ...
        if (res && mysql_num_rows(res) > 0) {
            MYSQL_ROW row = mysql_fetch_row(res);
            strcpy(out_name, row[0]);
        } else { strcpy(out_name, "Unknown File"); }
        if (res) mysql_free_result(res);
    } else { strcpy(out_name, "Error"); }
}
...
void get_unique_name(MYSQL *conn, int parent_id, char *original_name, char *out_name, int is_folder) {
    ...
    while (1) {
        ...
        if (parent_id > 0) sprintf(parent_val, "%d", parent_id);
...
int copy_physical_file(int src_id, int dest_id) {
    ...
    if (!f_dest) { fclose(f_src); return 0; }
...
void copy_recursive_inner(MYSQL *conn, int src_parent_id, int dest_parent_id, int owner_id) {
    ...
    if (num_rows == 0) { mysql_free_result(res); return; }

    struct NodeInfo { int id; char name[256]; char type[10]; long long size; };
...
void recursive_copy_for_zip(MYSQL *conn, int node_id, const char *base_path) {
    ...
    typedef struct { int id; char name[256]; char type[10]; } NodeInfo;
    NodeInfo *nodes = NULL;
...
void share_recursive(MYSQL *conn, int node_id, int target_user_id, const char *permission) {
    ...
    if (count > 0) {
        ...
        int i = 0;
        MYSQL_ROW row;
        while ((row = mysql_fetch_row(result))) child_ids[i++] = atoi(row[0]);
        mysql_free_result(result); 
        for (int k = 0; k < count; k++) share_recursive(conn, child_ids[k], target_user_id, permission);
        free(child_ids);
    } else {
        mysql_free_result(result);
    }
...
void remove_share_recursive(MYSQL *conn, int node_id, int target_user_id) {
    ...
    if (count > 0) {
        ...
        while ((row = mysql_fetch_row(result))) child_ids[i++] = atoi(row[0]);
        mysql_free_result(result); 
        for (int k = 0; k < count; k++) remove_share_recursive(conn, child_ids[k], target_user_id);
        free(child_ids);
    } else {
        mysql_free_result(result);
    }
...
int check_access_recursive(MYSQL *conn, int node_id, int user_id) {
    ...
    if (mysql_query(conn, query)) return 0;
    MYSQL_RES *res = mysql_store_result(conn);
    if (res && mysql_num_rows(res) > 0) { mysql_free_result(res); return 1; }
    if (res) mysql_free_result(res);

    sprintf(query, "SELECT parent_id, owner_id FROM nodes WHERE id = %d", node_id);
    if (mysql_query(conn, query)) return 0;
    res = mysql_store_result(conn);
    if (!res || mysql_num_rows(res) == 0) { if (res) mysql_free_result(res); return 0; }
    MYSQL_ROW row = mysql_fetch_row(res);
    int parent_id = (row[0]) ? atoi(row[0]) : 0;
    int owner_id = atoi(row[1]);
    mysql_free_result(res);

    if (owner_id == user_id) return 1;
...
int has_write_access(MYSQL *conn, int node_id, int user_id) {
    if (node_id <= 0) return 1; // ROOT của chính người dùng

    // Lấy owner của node
    char query[512];
    sprintf(query, "SELECT owner_id FROM nodes WHERE id = %d", node_id);
    if (mysql_query(conn, query)) return 0;
    MYSQL_RES *res = mysql_store_result(conn);
    if (!res || mysql_num_rows(res) == 0) { if (res) mysql_free_result(res); return 0; }
    MYSQL_ROW row = mysql_fetch_row(res);
    ...
    // Kiểm tra shared_nodes.permission có chứa "write"
    sprintf(query, "SELECT permission FROM shared_nodes WHERE node_id = %d AND shared_with_user = %d", node_id, user_id);
    if (mysql_query(conn, query)) return 0;
    res = mysql_store_result(conn);
    if (!res || mysql_num_rows(res) == 0) { if (res) mysql_free_result(res); return 0; }
    row = mysql_fetch_row(res);
    int allowed = (row[0] && strstr(row[0], "write") != NULL);
    mysql_free_result(res);
    return allowed;
}
...
void handle_login(int sock, MYSQL *conn, RequestPacket *req) {
    ...
    if (mysql_query(conn, query)) {
        ...
        if (result && mysql_num_rows(result) > 0) {
            ...
            res.status = CMD_ERROR; strcpy(res.message, "Invalid credentials");
...
void handle_create_folder(int sock, MYSQL *conn, RequestPacket *req) {
    ...
    int file_count = 0;
    char raw_name[256];
    strncpy(raw_name, req->arg1, 255); raw_name[255] = '\0';

    char *suffix = strstr(raw_name, "__UPLOAD_REQ__");
    if (suffix) {
        if (strlen(suffix) > 14) file_count = atoi(suffix + 14);
        *suffix = '\0'; 
        is_upload = 1;
    }
    ...
    char username[50]; get_username_by_id(conn, req->user_id, username);
    char parent_name[256]; get_node_name_by_id(conn, req->parent_id, parent_name);

    // Chặn tạo folder nếu không có quyền ghi trên parent
    if (!has_write_access(conn, req->parent_id, req->user_id)) {
        res.status = CMD_ERROR; strcpy(res.message, "Access denied");
        ...
        send_packet(sock, (char *)&res, sizeof(res)); return;
    }

    char parent_val[20] = "NULL";
    if (req->parent_id > 0) sprintf(parent_val, "%d", req->parent_id);
    ...
    if (mysql_query(conn, query)) {
        res.status = CMD_ERROR; strcpy(res.message, "DB Error");
...
void handle_list(int sock, MYSQL *conn, RequestPacket *req) {
    ...
    if (folder_id == 0 && req->node_id > 0) folder_id = req->node_id;
    ...
    if (folder_id == 0) { // ROOT
        ...
        if (check_access_recursive(conn, folder_id, req->user_id)) {
            sprintf(query, 
                "SELECT n.id, n.name, n.type, n.size, u.username, 0 as is_shared "
                "FROM nodes n JOIN users u ON n.owner_id = u.id "
                "WHERE n.parent_id = %d", folder_id);
        } else {
            int count = 0; send_packet(sock, (char *)&count, sizeof(int)); return;
        }
    }
...
void handle_search(int sock, MYSQL *conn, RequestPacket *req) {
    ...
    MYSQL_RES *result = mysql_store_result(conn);
    if (!result) { int count = 0; send_packet(sock, (char *)&count, sizeof(int)); return; }
    ...
    int i = 0;
    while ((row = mysql_fetch_row(result))) {
        if (check_access_recursive(conn, atoi(row[0]), req->user_id)) { valid_rows[i] = 1; valid_count++; }
        i++;
    }
...
void handle_get_path(int sock, MYSQL *conn, RequestPacket *req) {
    ...
    while (current > 0 && count < 100) {
        ...
        if (mysql_query(conn, query)) break;
        MYSQL_RES *res = mysql_store_result(conn);
        if (!res || mysql_num_rows(res) == 0) { if(res) mysql_free_result(res); break; }
        ...
        if (!check_ownership(conn, current, req->user_id)) {
            ...
            if (is_cur && !is_par) { count++; break; }
        }
        count++; current = pid;
    }

    PathNode reversed[100];
    for(int i=0; i<count; i++) reversed[i] = path[count-1-i];
    send_packet(sock, (char *)&count, sizeof(int));
    for(int i=0; i<count; i++) send_packet(sock, (char *)&reversed[i], sizeof(PathNode));
}
...
void handle_upload(int sock, MYSQL *conn, RequestPacket *req) {
    ...
    char filepath[512]; sprintf(filepath, "%s/%lld", SERVER_ROOT, file_id);
    FILE *fp = fopen(filepath, "wb");
    long long remaining = req->size; char buffer[4096];
    while (remaining > 0) {
        int to_read = (remaining > 4096) ? 4096 : (int)remaining;
        int n = recv(sock, buffer, to_read, 0);
        if (n <= 0) break;
        if (fp) fwrite(buffer, 1, n, fp);
        remaining -= n;
    }
    if (fp) fclose(fp);
...
void handle_download(int sock, MYSQL *conn, RequestPacket *req) {
    ...
    if (mysql_query(conn, query)) goto send_error;
    MYSQL_RES *result = mysql_store_result(conn);
    if(!result || mysql_num_rows(result) == 0) {
        if(result) mysql_free_result(result);
        char username[50]; get_username_by_id(conn, req->user_id, username);
    ...
    MYSQL_ROW row = mysql_fetch_row(result);
    char type[10]; strcpy(type, row[0]);
    char name[256]; strcpy(name, row[1]);
    mysql_free_result(result);

    char filepath[512];
    if (strcmp(type, "folder") == 0) {
        char temp_dir[256]; sprintf(temp_dir, "temp_export_%d", req->node_id);
        char cmd[1024];
        sprintf(cmd, "rm -rf %s && mkdir -p \"%s/%s\"", temp_dir, temp_dir, name); system(cmd);
        
        char base_path[512]; sprintf(base_path, "%s/%s", temp_dir, name);
        recursive_copy_for_zip(conn, req->node_id, base_path);
        
        sprintf(filepath, "temp_%d.zip", req->node_id);
        sprintf(cmd, "cd %s && zip -r -q ../%s .", temp_dir, filepath); system(cmd);
        sprintf(cmd, "rm -rf %s", temp_dir); system(cmd);
        is_temp_zip = 1;
    } else {
    ...
    if (fp) {
        ...
        while ((n = fread(buffer, 1, 4096, fp)) > 0) send_all(sock, buffer, n);
        fclose(fp);
    }
    if (is_temp_zip) unlink(filepath);
    return;

send_error:
    res.status = CMD_ERROR;
    if(strlen(res.message) == 0) strcpy(res.message, "Download Error");
    send_packet(sock, (char *)&res, sizeof(res));
}
...
void handle_rename(int sock, MYSQL *conn, RequestPacket *req) {
    ...
    if (!check_ownership(conn, req->node_id, req->user_id)) { 
        res.status = CMD_ERROR; strcpy(res.message, "Access denied"); 
        char username[50]; get_username_by_id(conn, req->user_id, username);
        char node_name[256]; get_node_name_by_id(conn, req->node_id, node_name);
        write_log("RENAME_FAIL: Access denied. User '%s' (ID %d) cannot rename '%s' (ID %d).", username, req->user_id, node_name, req->node_id);
        send_packet(sock, (char*)&res, sizeof(res)); return; 
    }
    
    char old_name[256]; char parent_name[256]; int parent_id = 0;
    // Lấy tên cũ và parent_id
    {
        char q[512]; sprintf(q, "SELECT name, parent_id FROM nodes WHERE id=%d", req->node_id);
    ...
    char query[512]; sprintf(query, "UPDATE nodes SET name = '%s' WHERE id = %d", req->arg1, req->node_id);
    ...
    if(mysql_query(conn, query)) { 
        ...
        res.status = CMD_SUCCESS; strcpy(res.message, "Rename OK"); 
        char username[50]; get_username_by_id(conn, req->user_id, username);
...
void handle_delete(int sock, MYSQL *conn, RequestPacket *req) {
    ...
    char name[256]; int parent_id = 0; char parent_name[256];
    {
        char q[512]; sprintf(q, "SELECT name, parent_id FROM nodes WHERE id=%d", req->node_id);
        if (mysql_query(conn, q) == 0) {
            MYSQL_RES *r = mysql_store_result(conn);
    ...
    char query[512]; sprintf(query, "DELETE FROM nodes WHERE id = %d", req->node_id);
    if(mysql_query(conn, query)) { 
        res.status = CMD_ERROR; strcpy(res.message, "DB Error"); 
        char username[50]; get_username_by_id(conn, req->user_id, username);
        ...
        res.status = CMD_SUCCESS; strcpy(res.message, "Delete OK"); 
        char username[50]; get_username_by_id(conn, req->user_id, username);
...
void handle_copy(int sock, MYSQL *conn, RequestPacket *req) {
    ...
    MYSQL_RES *r = mysql_store_result(conn);
    if (!r || mysql_num_rows(r) == 0) { 
        res.status = CMD_ERROR; strcpy(res.message, "Source not found"); 
        char username[50]; get_username_by_id(conn, req->user_id, username);
        write_log("COPY_FAIL: User '%s' (ID %d) attempted to copy missing source Node ID %d.", username, req->user_id, req->node_id);
        send_packet(sock, (char*)&res, sizeof(res)); return; 
    }
    MYSQL_ROW row = mysql_fetch_row(r);
    char src_name[256]; strcpy(src_name, row[0]);
    char src_type[20]; strcpy(src_type, row[1]);
    ...
    if (mysql_query(conn, query) == 0) {
        ...
        if (is_folder) copy_recursive_inner(conn, req->node_id, new_id, req->user_id);
        else copy_physical_file(req->node_id, new_id);

        res.status = CMD_SUCCESS;
        if (strcmp(src_name, final_name) != 0) snprintf(res.message, sizeof(res.message), "Copied as '%.200s'", final_name);
        else strcpy(res.message, "Copy success");
        ...
        res.status = CMD_ERROR; strcpy(res.message, "DB Copy Error");
        char username[50]; get_username_by_id(conn, req->user_id, username);
...
void handle_move(int sock, MYSQL *conn, RequestPacket *req) {
    ...
    char current_name[256]; char type[20];
    char query[512]; sprintf(query, "SELECT name, type FROM nodes WHERE id=%d", req->node_id);
    if (mysql_query(conn, query)) { 
        res.status = CMD_ERROR; strcpy(res.message, "DB Error"); 
        char username[50]; get_username_by_id(conn, req->user_id, username);
    ...
    MYSQL_RES *r = mysql_store_result(conn);
    MYSQL_ROW row = mysql_fetch_row(r);
    strcpy(current_name, row[0]); strcpy(type, row[1]);
    mysql_free_result(r);

    char new_name[256];
    get_unique_name(conn, req->parent_id, current_name, new_name, (strcmp(type, "folder") == 0));

    sprintf(query, "UPDATE nodes SET parent_id=%d, name='%s' WHERE id=%d AND owner_id=%d", 
            req->parent_id, new_name, req->node_id, req->user_id);

    if (mysql_query(conn, query) == 0) {
        if (mysql_affected_rows(conn) > 0) {
            res.status = CMD_SUCCESS;
            if (strcmp(current_name, new_name) != 0) snprintf(res.message, sizeof(res.message), "Moved and renamed to '%.200s'", new_name);
            else strcpy(res.message, "Move OK");
            
            char username[50]; get_username_by_id(conn, req->user_id, username);
            char parent_name[256]; get_node_name_by_id(conn, req->parent_id, parent_name); // Lấy tên cha
            ...
            res.status = CMD_ERROR; strcpy(res.message, "Move Failed"); 
            char username[50]; get_username_by_id(conn, req->user_id, username);
...
void handle_share(int sock, MYSQL *conn, RequestPacket *req) {
    ResponsePacket res; memset(&res, 0, sizeof(res));
    int has_right = check_ownership(conn, req->node_id, req->user_id);
    ...
    MYSQL_RES *result = mysql_store_result(conn);
    if (!result || mysql_num_rows(result) == 0) { 
        res.status = CMD_ERROR; strcpy(res.message, "User not found"); 
        char username[50]; get_username_by_id(conn, req->user_id, username);
    ...
    res.status = CMD_SUCCESS; strcpy(res.message, "Share OK");
...
void handle_get_share_list(int sock, MYSQL *conn, RequestPacket *req) {
    ...
    if (is_owner) sprintf(query, "SELECT u.username, sn.permission FROM shared_nodes sn JOIN users u ON sn.shared_with_user = u.id WHERE sn.node_id = %d", req->node_id);
    else sprintf(query, "SELECT u.username, sn.permission FROM shared_nodes sn JOIN users u ON sn.shared_with_user = u.id WHERE sn.node_id = %d AND sn.shared_with_user = %d", req->node_id, req->user_id);
    ...
    MYSQL_ROW row;
    while ((row = mysql_fetch_row(result))) {
        ShareEntry se; memset(&se, 0, sizeof(se));
        strcpy(se.username, row[0]); strcpy(se.permission, row[1]);
...
void handle_remove_share(int sock, MYSQL *conn, RequestPacket *req) {
    ...
    char query[512]; sprintf(query, "SELECT id FROM users WHERE username='%s'", req->arg1);
    mysql_query(conn, query);
    MYSQL_RES *r = mysql_store_result(conn);
    if (!r || mysql_num_rows(r) == 0) { 
        res.status = CMD_ERROR; strcpy(res.message, "User not found"); 
    ...
    res.status = CMD_SUCCESS; strcpy(res.message, "Unshared");
...
void *client_thread(void *arg) {
    ...
    MYSQL *conn = connect_db(); if(!conn) { close_socket(sock); return NULL; }
    RequestPacket req; int len;
    
    int current_user_id = -1;

    while(1) {
        char *data = recv_packet(sock, &len); if(!data) break;
        if ((size_t)len < sizeof(RequestPacket)) { free(data); continue; }
        memcpy(&req, data, sizeof(RequestPacket)); free(data);
        
        if (req.user_id > 0) current_user_id = req.user_id;

        switch(req.command) {
            case CMD_LOGIN: handle_login(sock, conn, &req); break;
            case CMD_REGISTER: handle_register(sock, conn, &req); break;
            case CMD_CREATE_FOLDER: handle_create_folder(sock, conn, &req); break;
            case CMD_LIST: handle_list(sock, conn, &req); break;
            case CMD_SEARCH: handle_search(sock, conn, &req); break;
            case CMD_GET_PATH: handle_get_path(sock, conn, &req); break;
            case CMD_UPLOAD_INIT: handle_upload(sock, conn, &req); break;
            case CMD_DOWNLOAD_INIT: handle_download(sock, conn, &req); break;
            case CMD_RENAME: handle_rename(sock, conn, &req); break;
            case CMD_DELETE: handle_delete(sock, conn, &req); break;
            case CMD_COPY: handle_copy(sock, conn, &req); break;
            case CMD_MOVE: handle_move(sock, conn, &req); break;
            case CMD_SHARE: handle_share(sock, conn, &req); break;
            case CMD_GET_SHARE_LIST: handle_get_share_list(sock, conn, &req); break;
            case CMD_REMOVE_SHARE: handle_remove_share(sock, conn, &req); break;
        }
    }
    ...
    mysql_close(conn); close_socket(sock); return NULL;
}
...
int main() {
    ...
    while(!stop_server) {
        char ip[50]; int client_sock = accept_client(server_socket_fd, ip);
        if (client_sock < 0) {
        ...
        if(client_sock >= 0) {
            ...
            pthread_t tid; pthread_create(&tid, NULL, client_thread, new_sock); pthread_detach(tid);
        }
    }
