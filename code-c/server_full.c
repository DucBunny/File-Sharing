...
// Xử lý lấy đường dẫn (Giữ nguyên)
void handle_get_path(int sock, MYSQL *conn, RequestPacket *req)
{
    int current_node_id = req->node_id;
    int user_id = req->user_id;
    char query[512];

    PathNode path[100]; // Giới hạn độ sâu 100
    int count = 0;

    // Lặp để truy vết ngược từ node hiện tại lên root
    while (current_node_id > 0 && count < 100)
    {
        // Lấy thông tin node hiện tại
        sprintf(query, "SELECT name, parent_id FROM nodes WHERE id = %d AND owner_id = %d",
        ...
        if (!result || mysql_num_rows(result) == 0)
        {
            if (result)
                mysql_free_result(result);
            break; // Dừng nếu node không tồn tại hoặc không thuộc sở hữu
        }

        MYSQL_ROW row = mysql_fetch_row(result);

        // Lưu thông tin vào mảng (tạm thời)
        path[count].id = current_node_id;
        ...
        if (row[1] != NULL)
        {
            current_node_id = atoi(row[1]);
        }
        else
        {
            current_node_id = 0; // Đã tới root
        }

        mysql_free_result(result);
    }

    // Đảo ngược mảng để có thứ tự từ Root -> Node hiện tại
    PathNode reversed_path[100];
    for (int i = 0; i < count; i++)
    {
        reversed_path[i] = path[count - 1 - i];
    }

    // Gửi số lượng node trên đường dẫn
    send_packet(sock, (char *)&count, sizeof(int));

    // Gửi từng PathNode
    for (int i = 0; i < count; i++)
    {
        send_packet(sock, (char *)&reversed_path[i], sizeof(PathNode));
    }
}
...
void handle_upload(int sock, MYSQL *conn, RequestPacket *req)
{
    ...
    // KIỂM TRA TYPE: Nếu tên file kết thúc bằng .zip, ta đặt type là 'folder' (đại diện cho zip)
    // Nếu không, mặc định là 'file'. (Tạm thời, server chỉ lưu file, không nén/giải nén)
    char node_type[10] = "file";
    if (strstr(req->arg1, ".zip"))
    {
        // Nếu client gửi file nén (tải lên thư mục), ta coi nó là một folder logic
        // Đây là cách đơn giản để đánh dấu trong DB rằng đây là một thư mục được nén.
        strcpy(node_type, "folder");
    }

    char query[512];
    sprintf(query, "INSERT INTO nodes (owner_id, name, type, parent_id, size) VALUES (%d, '%s', '%s', %s, %lld)",
            req->user_id, req->arg1, node_type, parent_val, req->size);
...
// Xử lý Tải xuống (ĐÃ SỬA: Cho phép tải xuống cả file và folder (dạng zip))
void handle_download(int sock, MYSQL *conn, RequestPacket *req)
{
    ...
    char query[512];
    // Lấy thông tin node (bao gồm kiểm tra quyền sở hữu/chia sẻ)
    ...
    MYSQL_ROW row = mysql_fetch_row(result);
    // BỎ QUA KIỂM TRA TYPE: Cho phép tải xuống bất cứ thứ gì có file vật lý
    // if (strcmp(row[0], "file") != 0) // ... (đã bị xóa)
...
void handle_delete(int sock, MYSQL *conn, RequestPacket *req)
{
    ...
    // 2. Xóa file vật lý nếu là FILE hoặc FOLDER (ZIP)
    if (strcmp(node_type, "file") == 0 || strstr(node_type, "folder") != NULL)
    {
        file_id = req->node_id;
