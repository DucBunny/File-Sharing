...
// Forward declarations from network_logic
extern void send_packet(int sock, const char *data, int len);
extern char *recv_packet(int sock, int *len);
extern int send_all(int sock, const char *data, int len);
...
void handle_register(int sock, MYSQL *conn, RequestPacket *req)
{
    ...
    if (mysql_query(conn, query))
    {
        ...
        {
            strcpy(res.message, "Tên đăng nhập đã tồn tại");
...
void handle_create_folder(int sock, MYSQL *conn, RequestPacket *req)
{
    ...
    else
    {
        ...
        snprintf(res.message, sizeof(res.message), "Đã tạo thư mục: %.200s", final_name);
    }
    send_packet(sock, (char *)&res, sizeof(res));
}
...
void handle_upload(int sock, MYSQL *conn, RequestPacket *req)
{
    ...
    res.status = CMD_SUCCESS;
    snprintf(res.message, sizeof(res.message), "Đã tải lên thành công '%.200s'", final_name);
    send_packet(sock, (char *)&res, sizeof(res));
}
...
void handle_download(int sock, MYSQL *conn, RequestPacket *req)
{
    ...
    if (!has_read_access(conn, req->node_id, req->user_id))
    {
        ...
        char username[50];
        get_username_by_id(conn, req->user_id, username);
        write_log("DOWNLOAD_DENY: user=%s(%d) lacks read permission on node_id=(%d) to download",
                  username, req->user_id, req->node_id);
        send_packet(sock, (char *)&res, sizeof(res));
        return;
    }
...
void handle_rename(int sock, MYSQL *conn, RequestPacket *req)
{
    ...
    {
        res.status = CMD_SUCCESS;
        strcpy(res.message, "Đổi tên thành công");
        char username[50];
        get_username_by_id(conn, req->user_id, username);
...
void handle_delete(int sock, MYSQL *conn, RequestPacket *req)
{
    ...
    {
        res.status = CMD_SUCCESS;
        strcpy(res.message, "Xóa thành công");
        char username[50];
        get_username_by_id(conn, req->user_id, username);
...
void handle_copy(int sock, MYSQL *conn, RequestPacket *req)
{
        ...
        !has_write_access(conn, req->parent_id, req->user_id))
    {
        ...
        write_log("COPY_DENY: User '%s' (ID %d) lacks write permission on Source '%s' (ID %d) or Destination '%s' (ID %d).",
                  username, req->user_id, node_name, req->node_id, parent_name, req->parent_id);
    ...
    if (mysql_query(conn, query) == 0)
    {
        ...
        if (strcmp(src_name, final_name) != 0)
            snprintf(res.message, sizeof(res.message), "Đã sao chép thành '%.200s'", final_name);
        else
            strcpy(res.message, "Sao chép thành công");
...
void handle_move(int sock, MYSQL *conn, RequestPacket *req)
{
        ...
        !has_write_access(conn, req->parent_id, req->user_id))
    {
        ...
        write_log("MOVE_DENY: User '%s' (ID %d) lacks write permission on Source '%s' (ID %d) or Destination '%s' (ID %d).",
                  username, req->user_id, node_name, req->node_id, parent_name, req->parent_id);
    ...
    if (mysql_query(conn, query) == 0)
    {
        if (mysql_affected_rows(conn) > 0)
        {
            ...
            if (strcmp(current_name, new_name) != 0)
                snprintf(res.message, sizeof(res.message), "Đã di chuyển và đổi tên thành '%.200s'", new_name);
            else
                strcpy(res.message, "Di chuyển thành công");
